...
let a = 'https://restcountries.com/v3.1/all',
  u = 'https://restcountries.com/v3.1/region/',
  d = 'https://restcountries.com/v3.1/name/',
  p = document.querySelector('.photo');
function f(n, t, i, o, e) {
  return `<div class="photo_box">
          <img src="${n}" alt="">
          <h1>${t}</h1>
          <div class="data">
            <div class="data_box">
...
g.addEventListener('change', (n) => {
  console.log(n.target.value), s(u + String(n.target.value.toLowerCase()));
});
let h = document.querySelector('input');
h.addEventListener('input', (n) => {
  let t = d + n.target.value.toLowerCase();
  s(t);
});
let v = document.querySelector('.header_second');
